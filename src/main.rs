...
// eg `cargo run -- --git-dir="/Users/edward.andrewshodgson/Developer/work/dashboards-and-visualisations" --author="Edward Andrews-Hodgson"`
    ...
    let revwalk = revwalk
        .filter_map(|id| {
            ...
            let then = now
                ...
                .unwrap()
                .as_secs() as i64;
            if commit.time() < Time::new(then, 0) {
                return None;
            }
            Some(Ok(commit))
        })
